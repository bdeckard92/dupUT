...
* Open the [server.js](Unsolved/server.js) file and do the following:
  ...
  * In the `/` route, if the `countPage` session variable exists, increment it by 1 and set the `firstTime` session variable to `false`.

  * If the `countPage` session variable doesn't already exist, set it to 1 and set the `firstTime` session variable to `true`.

  * In the `data` object, include the `books` array, `countPage` and `firstTime` session variables to be sent over to `index.handlebars`.
