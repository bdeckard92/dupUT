...
* Navigate to `03-Blog-CRUD/Solved/` in your command line and run `npm start`. Navigate to `localhost:3001` in your browser and demonstrate the following:
    ...
    * 🔑 We can edit a post by clicking on it

    * 🔑 We can delete a post by click on the x
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We've used CRUD methods before but this time we will specifically use Sequelize methods.
...
* Open `03-Blog-CRUD/Solved/models/post.js` in your IDE and explain the following: 
    ...
    * 🔑 We need to add a few validations to our `Post`. First we need to prevent NULL values from being entered in the title and body:
...
* **Let students know that this activity is simple and will be given only 10 minutes to complete**
...
* Open `04-Blog-Association/Solved/models/index.js` in your IDE and explain the following: 
    ...
    * 🔑 We need to create a relationship that instructs the `Post` to belong to an `Author`. `Post` is the source model and the foreign key will be defined in it. 

    ```js
    Post.belongsTo(Author);
    ```

    * 🔑 We need to create a relationship that instructs `Author` that it has many `Post`. Here `Post` is the target model with the foreign key. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to the lesson plan, read the [Sequelize Manual on Associations](https://sequelize.org/master/manual/assocs.html), and stick around for office hours to ask for help.
...
* Navigate to `05-Blog-CRUD/Solved/` in your command line and run `npm start`. Navigate to `localhost:3001` in your browser and demonstrate the following:
...
* **Let students know that this activity is simple and will be given only 15 minutes to complete**

* Divide students into breakout rooms.

* Direct students to the activity instructions found in `05-Blog-JoinsREADME.md`.
...
* Open `05-Blog-Joins/Solved/routes/api/post-api-routes.js` in your IDE and explain the following: 
    ...
    * Notice how we are requiring our models at the top of the file. It is a little different from how we've been doing it in our previous activities today.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to the lesson plan, read the [Sequelize Manual on Eager Loading](https://sequelize.org/master/manual/eager-loading.html), and stick around for office hours to ask for help.
