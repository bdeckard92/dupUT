...
* Open `04-One-to-One/Solved/package.json` in your IDE and explain the following:
  ...
  * In this activity, we included a "watch" script in the `package.json` file to automatically run `nodemon server` command when we make changes to the project. This allows us to see the changes we make to the project in real time by making use of the `nodemon` command line tool.

  * `nodemon` is a tool that automatically restarts our Node.js server when we make changes to our code.
