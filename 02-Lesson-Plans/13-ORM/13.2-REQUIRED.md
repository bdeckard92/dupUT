...
* The `03-Update-Delete` activity includes a `/seed` POST route simply to make it easier to populate the database with books quickly.
...
* Lesson 1 covered CRUD methods and associations in Lesson 3 and 4.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We used `update` and `delete` methods in our `User` model in Lesson 1 and in our `Post` model in Lesson 3.
...
* Navigate to `04-One-to-One/Solved` and run `npm i` from the command line. Be sure to change the `.env.EXAMPLE` file into your own `.env` file with your credentials. Then run `npm start` from the command line and demonstrate the following:
  ...
  * 🔑 By default, Sequelize outputs the commands that it performs on the SQL server to the console, even on load. You can see that in the following example:
  ...
  * In these statements that create the tables, we see that Sequelize is using the default database. We can change this by using the `database` option in the `.env` file. We can see that the `library_card` table stores a reference to the `reader` table.

* Open `04-One-to-One/Solved/package.json` in your IDE and explain the following:

  * In this activity, we included a "watch" script in the `package.json` file to automatically run `nodemon server` when we execute `npm run watch`. This allows us to see the changes we make to the project in real time by making use of a utility called `nodemon`.

  * `nodemon` is a tool that automatically restarts our Node.js server when we make changes to our code. It can be helpful when we are making changes to our code, but don't want to have to restart the server every time we make a change.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Check out the [Sequelize documentation on associations](https://sequelize.org/master/manual/assocs.html).
...
* Use Insomnia to demo the following:
  ...
  * 🔑 In our `models/index.js` file, we can import the `Book` model, and associate it with the `Reader` model using a sequelize association method. We also do something similar for library cards and readers.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ What would happen if we try to associate a library card with a reader that doesn't exist?

  * 🙋 We get a `SequelizeForeignKeyConstraintError` error.

  * ☝ What Sequelize methods can we use to associate a reader with a library card?

  * 🙋 We can use the `hasMany` method to associate a reader with multiple library cards.

  * ☝ In our query, how could we retrieve the associated books for a reader?
