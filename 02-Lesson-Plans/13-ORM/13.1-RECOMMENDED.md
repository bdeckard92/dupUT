...
* Last week, they should have submitted their resume. Encourage students that have not yet done so to get that turned in to a Profile Coach as soon as possible so they can receive feedback on it.
...
* Ask the class the following questions (☝) and call on students for answers (🙋):
    ...
    * 🙋 We would use a `if-else` statement to compare the two values:
