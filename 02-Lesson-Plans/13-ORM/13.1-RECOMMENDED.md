...
* Open `01-Sequelize-Library/Solved/app/routes/api-routes.js` in your IDE and explain the following: 
    ...
    * 🔑 We use the `where` option to filter the query; for example, if we want to get the books of a specific genre or author:
    ...
    * 🔑 We use Sequelize Operators, such as `gte` or `lte` to make number comparisons. For example, if we want to get the books that have more than 150 pages or less than 150 pages:
...
* Open `@TODO/folder/file` in your IDE and explain the following: 
    ...
    * 🔑 We use the Sequelize `destroy()` method inside the DELETE route to delete the todo with the id available to us in `req.params.id`.
    ...
    * 🔑 We use the Sequelize `update()` method inside the PUT route to update a todo with the new todo data sent to the server in `req.body`.
