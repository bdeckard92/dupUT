# Module 01 Flex Session: Git & HTML
...
- Students will struggle adapting to the command line. The associated activity may not take the entire time allotted, so use any remainder to demonstrate other commands such as `cd`, `pwd`, `cat`, etc.
