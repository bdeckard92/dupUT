...
* Open `01-Actions-Reducers/Solved` in your IDE and demonstrate the following:
    ...
    * 🔑 First, copy the [src](../../01-Class-Content/22-State/01-Activities/01-Actions-Reducers/Solved/src) folder and paste it into the `Book-Lounge` app inside the `client` folder. 
    ...
    * 🔑 We see a list of books. If we click one, it will render a new page with details of the book. Notice the header also changes to the title of the book you are viewing. When you go back to the list of books, the header also changes to "Now Viewing: Your book list". 
    
* Ask the class the following questions (☝️) and call on students for the answers (🙋):

    * ☝️ Where have we used this before?

    * 🙋 The functionality of the app shouldn't be affected by the implementation of actions and reducers. But we learned about actions and reducers in Lesson 1 as we prepared to implement global state to our `Shop-Shop` app.
...
* Open `01-Actions-Reducers/Solved/src/utils/actions.js` in your IDE and explain the following:
    ...
    * Think about what kind of actions we will need to take in order to access or update the state. That should drive you to create the right actions. In our case, we need to access all of the books and also one current book that the user chooses to view.
    ...
    * 🔑 `UPDATE_BOOKS` will get the list of books and store it in our global state. `UPDATE_CURRENT_BOOK` will set the `currentBook` to what the user chose. Now let's write the reducers to go with these actions!
...
* Open `01-Actions-Reducers/Solved/src/utils/reducers.js` in your IDE and explain the following:
    ...
    * 🔑 Next we create the `reducer()` function that takes in the state and an action. We use a `switch` statement to compare the value of the `action.type` with the possible actions. 
...
* Open `02-Test-Reducers/Solved` in your IDE and demonstrate the following:
    ...
    * 🔑 Navigate to the [client](../../01-Class-Content/22-State/01-Activities/Book-Lounge/client) directory in your command line and run `npm run test`.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We will need to write two tests for both of our reducers.
