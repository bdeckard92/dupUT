...
* Open `03-Global-State/Solved` in your IDE and demonstrate the following:
  ...
  * 🔑 First, we copy the [src](../../01-Class-Content/22-State/01-Activities/03-Global-State/Solved/src) folder and paste it into the `00-practice-app` app inside the `client` folder.

  * 🔑 Then, we navigate to `00-practice-app` in our command line and run `npm install` and `npm run seed` to set up the app.
...
* Open [App.js](../00-practice-app/client/src/App.js) and import the `StoreProvider` so that the global state will be available to all of the components.
