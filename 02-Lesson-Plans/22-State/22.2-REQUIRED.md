...
* **Important**: To avoid continuous NPM installs, all of this today's activities' Solved and Unsolved folders include only a `src` folder. Swap out the `src` folder for each activity in `00-practice-app`. It's recommended that you completely restart the dev server between activities.

* This week's Challenge is refactoring the `Shop-Shop` e-commerce platform that students worked on throughout this module to use Redux. They will need to read through the official Redux documentation by themselves and find the right information to help them accomplish their task.
...
* Reducers and actions were all covered in Lesson 1.
...
* Open `04-Actions/Unsolved/utils/actions.js` in your IDE and explain the following:
  ...
  * In this activity, you will be annotating the actions in `actions.js` to help other developers understand what each action, and the corresponding reducer, is doing. This is often good practice in order to make sure that other developers understand the application's behavior.
...
* Direct students to the activity instructions found in `04-Actions/README.md`.
...
* Direct students to the activity instructions found in `05-useReducer/README.md`.
