...
* Open `04-Actions/Unsolved/utils/actions.js` in your IDE and explain the following:
  ...
  * In the next activity, you will be annotating the actions in `actions.js` to help other developers understand what each action, and the corresponding reducer, is doing. This is often good practice in order to make sure that other developers understand the application's behavior.
...
* Copy the `src` directory from `09-Ins_useReducer` and paste it into `00-practice-app`. Navigate to `00-practice-app` in your command line and run `npm install` and `npm start`.
  ...
  * To use the `useReducer` hook, we begin by importing the reducer and the `useReducer` Hook into the component:
  ...
  * In the next activity we will finish the implementation of the `useReducer` hook in our app.
...
* Remind students to copy the `src` directory from `05-useReducer/Unsolved` into their `00-practice-app` folder.
...
* Be sure that you have copied the `src` directory from `05-useReducer/Unsolved/` into `00-practice-app`.
