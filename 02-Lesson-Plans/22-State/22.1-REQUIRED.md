...
* In order to avoid continuous npm installs, all of this week's activities' Solved and Unsolved folders include only a `src` folder. For each activity, swap out the `src` folder in `00-practice-app`. It's recommended that you completely restart the dev server between activities.
...
* Direct students to the activity instructions found in `01-Providers/README.md`, which are also shown here:
  ...
  ````md
  # 🏗️ Implement Global State Using the React Context API
...
* Direct students to the activity instructions found in `02-Consumers/README.md`, which are also shown here:
  ...
  * It is done when changes in global state are reflected in necessary components.
