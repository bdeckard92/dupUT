...
* Open `01-Providers/Solved` in your IDE and demonstrate the following:
  ...
  * 🔑 The React Context hook returns a value called `Provider` and a `Consumer` which are both React components. For now, let's focus on the `Provider` component and work with the `Consumer` component later.
