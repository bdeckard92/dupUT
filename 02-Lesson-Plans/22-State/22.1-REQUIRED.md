...
* **Important**: To save time in class, please create a central practice app for the activities, similar to the one you may have used in the React unit. To do this, navigate to `fullstack-online/01-Class-Content/22-State/01-Activities` and run `npx create-react-app 00-practice-app`. The app will be ignored by git, as it is included in the `.gitignore` file already.
