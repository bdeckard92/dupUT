...
In today's recommended virtual class, students will utilize the `.map()` method in order to render JSX from an array of objects. Students will also use the React `useState` Hook to use state variables in a functional component.
