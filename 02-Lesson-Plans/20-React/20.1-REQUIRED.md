...
* **Important**: In order to avoid continuous npm installs, all of today's activities' `Solved` and `Unsolved` folders include only a `src` folder. Swap out the `src` folder for each activity in `00-practice-app`. It is recommended that you completely restart the dev server between activities.
