...
* Open `03-useEffect-Thermostat/Solved` in your IDE and demonstrate the following:
    ...
    * 🔑 First, we will need to copy the [src](../../01-Class-Content/20-React/01-Activities/03-useEffect-Thermostat/Solved/src) folder and paste it into the [reactpractice](../../01-Class-Content/20-React/01-Activities/reactpractice) app. 
...
* Open `04-Conditional-Render/Solved` in your IDE and demonstrate the following:
    ...
    * 🔑 Again, we will need to copy the [src](../../01-Class-Content/20-React/01-Activities/04-Conditional-Render/Solved/src) folder and replace the `src` folder in the [reactpractice](../../01-Class-Content/20-React/01-Activities/reactpractice) app with this one.

    * 🔑 Then, we navigate to [reactpractice](../../01-Class-Content/20-React/01-Activities/reactpractice) and start the app in dev mode by running `npm start` from the command line.

    * 🔑 This will automatically open the browser to <http://localhost:3000>.

    * 🔑 We see a navbar on top, and when we click on one, the page is rendered under the navbar.
...
* Open `05-Friends-List/Solved` in your IDE and demonstrate the following:
    ...
    * 🔑 Again, we will need to copy the [src](../../01-Class-Content/20-React/01-Activities/05-Friends-List/Solved/src) folder and replace the `src` folder in the [reactpractice](../../01-Class-Content/20-React/01-Activities/reactpractice) app with this one.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We will need to `map` and `filter` through objects to render them onto the page.
...
* Open `05-Friends-List/Solved/src/App.js` in your IDE and explain the following: 
    ...
    * 🔑 We use the `.filter()` method to create a new array. Then we set the `friendsList` to this new array.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 By using GraphQL, we're showing employers our knowledge and skill in a new API tool that's recently gained a lot of attention in web development. GraphQL is used as an alternative to RESTful API design patterns. This gives us the opportunity to review and expand upon previously learned topics such as Mongoose and React, but also learn new tools and concepts to make us more well-versed as developers.
