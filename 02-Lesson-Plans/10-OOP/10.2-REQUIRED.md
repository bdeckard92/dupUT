...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 ES6 classes must be called with the `new` keyword. With ES5 constructors, you could call `Car()` on its own. The other difference is that ES6 classes are not hoisted. 
