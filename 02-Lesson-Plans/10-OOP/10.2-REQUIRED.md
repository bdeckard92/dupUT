...
* Help students to see the bigger picture with the activities. While they may be struggling, learning ES6 Classes is useful for those with an interest in mobile development and this exposure prepare them to transition to other languages such as Java, C++, and Swift after boot camp. Likewise, Test Driven Development is a great way to practice breaking down a coding challenge into manageable chunks and coding with a plan. 

* If you need a quick brushup on ES6 Classes prior to class, [MDN Web Docs on classes](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes) provides a great review the basic components of classes. 
...
* Navigate to `03-First-Mock/Solved/` in your command line and run `node index.js` to demonstrate the following:
    ...
    * 🔑 When we run `node index.js`, we see a printout of the names and ages of children at day care. We also notice the list changes after an event occurs, like `Picked up Amir from day care`. 
    ...
    * These console logs are essentially to the functioning of our app. Using Jest, we can spy on the functionality of these console logs to make sure they are actually working as expected. 
...
* Navigate to `03-First-Mock/Solved/test/dayCare.test.js` in your IDE to demonstrate the following:
    ...
    * 🔑 Next, we use the `expect()` method to expect the console.log to be called and for it to return a message in the log. Because we are mocking the `console.log()` method, nothing will be printed to the console when the test is run, and we can spy on values provided to the `console.log()`. 
    ...
    * 🔑 Finally, after we run our test to spy on our console.log functionality, we want to restore our original functionality. To do so, we use the `mockRestore` method. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 When we use the `spyOn` method, we have a way to track existing functions and the behavior of our code. Although not required, this is a useful feature that we can to test the functionality of our Challenge code. Learning mocking helps us think about the behavior and outcomes of our code. This will help us write better, more predictable code!
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can use mocks to replace code with side effects, like printing to the console, making `fetch()` requests, and reading or writing to the file system so that these things do not happen when tests are being run.
...
* Navigate to `04-Classes/Solved/` in your command line and run `node character.js` to demonstrate the following:
    ...
    * 🔑 ES6 Classes are blueprints for creating objects.
    ...
    * 🔑 When we create a Class, we are creating a template that describes the characteristics and functionality for a group of objects. To actually create an object, we use the `new` operator and assign values. The new character will have the name `Grace`, have `30` strength and `75` hitPoints. 

      ```js
      const grace = new Character('Grace', 30, 75);
      ```

    * Once we have created a new character, `grace`, we can then access the methods. To check if our new character is out of hitPoints and no longer alive, we call the `.isAlive()` method on our new character. In this case we are actually checking if the `grace` character is not alive, so the not `!` operator has been added. 
...
* In Javascript, when we write an ES6 Class, the class should describe one object. To get the full picture, though, you sometimes need to add multiple objects to your code. 

* Navigate to `05-Multiple-Classes/Solved/` in your command line and run `node index.js` to demonstrate the following:

    * 🔑 In this command line app, whe have built an interface for Big Al's Toy Barn. Taking a look at the output, it looks like we may need to have more than one class to create blueprints for the objects.

* Navigate to `05-Multiple-Classes/Solved/toy.js` in your IDE to demonstrate the following:

    * 🔑 Let's start small. The toy store is going to need to have toys. In addition, each toy needs to have a name, a price and a count.
    ...
    * 🔑 We also use the `new` operator to create two toy instances in an array and export everything so we can use it in other parts of our code. 
    ...
    * Notice that we are keeping all the code needed for the toys in a separate file. This modular method will help keep our code clean and make it much easier to write unit tests for our code. 

* Navigate to `05-Multiple-Classes/Solved/store.js` in your IDE to demonstrate the following:

    * 🔑 Now to sell our toys, we need a store. We declare a store class, initialize it with a constructor and give it some methods that will be used to perform tasks. 
...
* Navigate to `05-Multiple-Classes/Solved/index.js` in your IDE to demonstrate the following:
    ...
    * 🔑 Now that we have both our toy objects and our Store class, we can put it all together. We create a new instance of the toy and set the stock value to to be our array of toys. 

      ```js
      const store = new Store("Big Al's Toy Barn", toys);
      ```

    * Now the store will be populated by our toy objects. In addition, by putting each object in its own file, we have made our code easier to read and easier to test. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 For the challenge, we will be required to create multiple classes and to keep our code organized it important to maintain separation of concerns. The code is easier to navigate if we give each class a clear responsibility.

    * ☝️ How would we build this?

    * 🙋 When we plan an app, we first thing about each object that we will need to use and how they may interact. Then, we declare each class with the class keyword and initialize each class with a constructor function. 
...
* Open `05-Multiple-Classes/Solved/store.js` in your IDE and explain the following:
  ...
  * Now, that we have spent time reverse engineering the code, we can take a closer look. 

  * We can create a new store by passing in the `name` and `stock` properties. The constructor will set `revenue` to `0`. When we initialize a new instance of the store, we will give the store a name and set the value of `stock` to be equal to our toy objects. 
...
* Open `05-Multiple-Classes/Solved/store.js` in your IDE and explain the following:
  ...
  * Now that the our new store has access to our toy objects, we simply call the store's methods.
