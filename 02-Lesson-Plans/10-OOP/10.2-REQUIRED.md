...
* Navigate to `05-Multiple-Classes/Solved/index.js` in your IDE to demonstrate the following:
    ...
    * 🔑 Now that we have both our toy objects and our Store class, we can put it all together. We create a new instance of the store, give it the name `Big Al's Toy Barn` and set the stock value to to be our array of toys. 
...
* Open `05-Multiple-Classes/Solved/store.js` in your IDE and explain the following:
  ...
  * We can create a new store by passing in the `name` and `stock` properties. For each new instance the `Store` class, the constructor will set `revenue` to `0`. When we initialize a new instance of the store, we will give the store a name and set the value of `stock` to be equal to our toy objects. 
