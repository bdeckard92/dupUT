...
* Navigate to `01-TDD/Solved` in your command line and run `npm install` and `npm run test` to demonstrate the following: 
    ...
    * 🔑 We start by stating the expectation, or the task we need the code to perform for the test to pass.  For example, in our first test we expect that the algorithm `isPalindrome1` should return false if the string is not a palindrome.
