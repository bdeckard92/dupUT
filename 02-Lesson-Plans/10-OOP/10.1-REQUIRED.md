...
* If you notice that your students are struggling with the activities or feeling frustrated, reassure them that these are hard topics to understand. These skills will get easier with time and practice, making students more employer-competitive! If you are comfortable doing so, share your own experiences and struggles learning and working with OOP and test-driven development.

* Emphasize the relevance and importance of the activities, even if students find them difficult. Test-driven development is a useful programming practice because it reinforces the practice of breaking a coding challenge into manageable chunks, as well as coding with a plan -- and it will ultimately help students build leaner, more predictable code.

* If you need to review test-driven development using Jest, refer to the [Jest documentation](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes).
...
* Open the lesson's [slide deck](https://docs.google.com/presentation/d/1O_Or8Fv6twbV4V9mIllFGczAqcdMzb5Mw2nzv94Cgfw/edit?usp=sharing) and review the following slides on foundation setting:
    ...
    * Ask the class the following questions (☝️) and call on students for the answers (🙋):
      ...
      * 🙋 Because it means that you can apply to more than just JavaScript roles! You could also apply for jobs using C# or Java and lean into the object-oriented programming experience for the application and interview.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Remind students that they are adding another layer to the full-stack skill set. It's important to consider what they have accomplished and acknowledge that it is a lot! It’s also okay to feel overwhelmed&mdash;as long as they don’t give up.
...
* Navigate to `01-TDD/Solved` in your command line and run `npm install` and `npm run test` to demonstrate the following:
  ...
  * 🔑 We start by stating the expectation, or the task that the code needs to perform for the test to pass. For example, in the first test we expect that the algorithm `isPalindrome1` should return `false` if the string is not a palindrome:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 In the Challenge, we will use test-driven development to write unit tests for each part of the code and ensure that each test passes.
...
* Open `01-TDD/Solved/test/algo.test.js` in your IDE and explain the following:
  ...
  * 🔑 Finally, we pass `result` as an argument into `expect()` and compare it to the value of `reversed` using the `toEqual()` method&mdash;meaning that we expect the value of `result` to equal the value of `reversed`:
  ...
  * 🔑 The `describe()` statement for `isPalindrome()` contains two tests because we needed to use `it()` twice&mdash;once to test that `isPalindrome()` returns `true` when a string is a palindrome and once to return `false` when it is not:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to Lesson 1, read the [Jest API documentation](https://jestjs.io/docs/en/api), and stay for Office Hours to ask for help.
...
* Open `02-Pass-Tests/Solved/algo.js` in your IDE and explain the following:
  ...
  * 🔑 When we write the code, we start by thinking about the steps needed to make the tests pass. We can write these steps as comments in the code, as a reminder of the code we will need to write -- to help ensure that we follow the plan:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Once the unit tests are written, we use JavaScript to write the algorithm code. To test whether the code passes, we run `npm run test`. If the test does not pass, we refactor until the code passes.
...
* Open `02-Pass-Tests/Solved/algo.js` in your IDE and explain the following:
  ...
  * Inside the `isPalindrome()` function, we use `reverse()` as a helper function to check whether `str` is the same when it is reversed (otherwise known as a **palindrome**):
...
* Run `npm run test` in the console and demonstrate the following:
  ...
  * 🔑 All of the tests now pass when we run `npm run test`, including `capitalize()`:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to Lessons 1 and 2, read the [Jest API documentation on getting started](https://jestjs.io/docs/en/getting-started), and stay for Office Hours to ask for help.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We covered test-driven development, or TDD, and how to write tests, as well as code that will pass those tests.
...
* Expect that students might ask for the following:
