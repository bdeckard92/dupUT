...
* Navigate to `01-TDD/Solved` in your command line and run `npm install` and `npm run test` to demonstrate the following: 
    ...
    * 🔑 When we use the Test Driven Development approach, we write unit tests that describe the expected functionality of the code before writing the actual code. This helps us define exactly what we need to happen with our app and then write only the code we need to meet our expectations. 
