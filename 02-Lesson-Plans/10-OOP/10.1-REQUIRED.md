...
* Navigate to `01-TDD/Solved/` in your command line and run 'npm install' and 'npm run test' to demonstrate the following: 
    ...
    * 🔑 When we use the Test Driven Development approach, we write unit tests that describe the expected functionality of the code before writing the actual code. 
    ...
    * We see that the tests are still failing. That is okay. In TDD, we start with failing tests and then write the code needed to make them pass. For this first step, though, we are only focused on writing the tests. We will write the code to make the tests pass in the next activity. 
 ...
 *  Open `02-Pass-Tests/Solved/algo.js` in your IDE and explain the following: 
    ...
    * 🔑 When we write our code, we start by thinking about the steps needed to make our tests pass. It is a good idea to write these steps as comments in our code to remind us of what code we will need to write.
    ...
    * 🔑 Once we have the steps needed to write our code to pass our test, we write the code to meet each step and ultimately make each test pass.
    ...
    * 🔑 To check if our code solution passes the test, we run 'npm run test'. If the test fails, we refactor and try again. This process of testing and refactoring is an essential step in Test Driven Development. 
