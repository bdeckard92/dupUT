 ...
 *  Open `02-Pass-Tests/Solved/algo.js` in your IDE and explain the following: 
    ...
    * 🔑 To check if our code solution passes the test, we run `npm run test`. If the test fails, we refactor it and try again. This process of testing and refactoring is an essential step in Test Driven Development and is an important skill for developers to practice to help them write more durable and leaner code. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 For the Challenge, we will be writing unit tests in Jest and then making sure our code passes each of the tests. 
