...
* Open `02-Pass-Tests/Solved/test/algo.test.js` in your IDE and explain the following:
  ...
  * 🔑 Let's repeat the TDD cycle one last time, with the third and final test. The `capitalize()` method includes one test, which verifies that it `"should take a string and return a new string with the first letter of each word capitalized"`:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 All of it! The field of web development is wide and rapidly evolving. Unless you choose to specialize, it's not necessary to completely know any one area to be successful.
