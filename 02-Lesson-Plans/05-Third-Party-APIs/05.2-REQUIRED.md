...
In today's virtual class, students will review Bootstrap and jQuery basics. Students will use the Bootstrap grid layout and components to quickly scaffold a UI for a webpage. Students will also see how jQuery UI widgets and interactions make a webpage more user-friendly.
...
* Take a few minutes before class to get familiar with both the [Bootstrap](https://getbootstrap.com/) and [jQuery UI](https://jqueryui.com/) documentation, as students will rely heavily on these resources in class and might need a nudge in finding answers.
...
* Reiterate often that, like CSS, using Bootstrap classes requires a bit of trial and error to achieve the intended design and layout. Encourage students to try using different variations of classes to become more comfortable with the process and learn more about their personal tastes.

* Emphasize that whenever we use a new library, framework, or technology, it's best that we familiarize ourselves with the official documentation as much as possible.
...
Encourage students to work on the remaining module algorithms during Office Hours. Review the algorithm solutions 5–10 minutes before class begins during Office Hours.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
	...
	* ☝️ Did anyone have an "A-ha" moment this week? When and why did it click?
...
* Open `03-Bootstrap-Layout/Solved/index.html` in your browser and explain the following:
	...
	* 🔑 If we resize the browser or use the `Toggle device toolbar` in the Chrome DevTools, we see that the grid layout responds accordingly.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can automatically size columns by using simple `col` classes and reorder them, among many other things!

	* ☝️ What can we do if we don't completely understanding this?

	* 🙋 We can refer to Lesson 2, read the [Bootstrap documentation on the grid](https://getbootstrap.com/docs/4.5/layout/grid/), and attend Office Hours to ask for help. Use the Bootstrap examples provided in the documentation to give yourself a starting point, then continue to tweak the class names to configure different layouts. Having a good understanding of CSS flexbox properties will ultimately be the best way to learn it.
...
* Open `04-Bootstrap-Utilities/Solved/index.html` in your IDE and explain the following:
  ...
  * 🔑 Next, we added classes to use sizing properties to make the row run the full width of the page, and used flex properties to center-justify content.
...
* Open `05-jQuery-UI/Solved/assets/js/script.js` in your IDE and explain the following:
  ...
  * Then we look for the element that has an id of `skill-name` (which, in this case, is the `<input>` element) and apply the `.autocomplete()` function to it. This will check if what is being typed is one of the values of the `skillNames` array that we just created.
  ...
  * 🔑 However, we also want to add the option of seeing the whitespace between the items shift. According to the ["Drop placeholder" option](https://jqueryui.com/sortable/#placeholder), we can do just that.
