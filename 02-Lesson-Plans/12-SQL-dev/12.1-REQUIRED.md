...
* Navigate to `01-SQL-Schema/Solved/` in the command line. and initialize the MySQL command line by typing `mysql -u root -p`. Enter your MySQL password. 
    ...
    * 🔑 We execute the `schema.sql` file using the `SOURCE` command.

      ```cmd
      SOURCE db/schema.sql;
      ```

    * 🔑 We verify that the database has been created using the `SHOW` command.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We will create a connection between the database server and the database for the Challenge. 
...
* Open `01-SQL-Schema/Solved/schema.sql` in your IDE and explain the following:
    ...
    * 🔑 We first type the SQL command to drop the database if it already exists. Then we create the database and tell MySQL to use that database, using the following SQL commands:
    ...
    * 🔑 We create the `books` table by using the `CREATE TABLE` SQL command. We specify that the `id` will be an integer which will act as the primary key and will need to be auto incremented. The `authorId` is also an integer but `title` should be a variable character:
    ...
    * 🔑 We create the `authors` table by using the `CREATE TABLE` SQL command:
    ...
    * 🔑 To initiate the MySQL Shell, type: `mysql -u root -p` in the command line and then your MySQL password. We create the `books_db` database by executing the `source schema.sql` command:
