...
* Open `01-SQL-Schema/Solved/schema.sql` in your IDE and explain the following:
   ...
   * 🔑 To create a table, we use the `CREATE` command. We then add one or more columns to hold our data. 

   * 🔑 Tables can hold a variety of data types. For each column, we assign the data type we want to store. 
...
* Open `01-SQL-Schema/Solved/schema.sql` in your IDE and explain the following:
    ...
    * 🔑 We use the `CREATE` command to add a second table. In my case, the table is called `categories`. We use `VARCHAR(30)` to store text up to 30 characters. For longer text, we use the `TEXT` data type. 
