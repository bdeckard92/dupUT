...
* Open `01-SQL-Schema/Solved/schema.sql` in your IDE and explain the following:
    ...
    * 🔑 For each field in the `books` table, we add a data type. For `VARCHAR` types, we add the number of characters allowed in the field. For `DECIMAL` types, we add the number of significant digits and the number of digits allowed after the decimal point. 
    ...
    * We use the `DESCRIBE` command in the MySQL Shell to test that the new tables `books` and `categories` exist and have the right data types.
