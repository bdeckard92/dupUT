...
* Navigate to `03-SQL-Constraints/Solved/` in the command line and demonstrate the following:
  ...
  * 🔑 We execute in the MySQL Shell using the `SOURCE` command.

  * 🔑 To see the types of data in our `books` table and the constraints, we use the `DESCRIBE` command. 

  * 🔑 We use constraints to define rules for our table's fields. 

  * 🔑 We use the `Primary Key` constraint to identify which field will be used to uniquely identify each row.

  * 🔑 We use the `Foreign Key` constraint to identify a field from a table that we will use to link two tables together. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We use the `primary key` to identify the field that will uniquely identify the table. We use the `foreign key` to define the field that will be used to link two tables.
...
* Open `03-SQL-Constraints/Solved/db/schema.sql` in your IDE and explain the following: 
  ...
  * 🔑 We use the `PRIMARY KEY` constraint to set up the unique identifier for the table. It is often the `id` field of our table. It is important that the field that is designated as the `PRIMARY KEY` has unique data for each row. 
  ...
  * 🔑 In our `books` table, we want our `price` field to link to our `prices` table. We use the `FOREIGN KEY` restraint to set the `price` field to link to another table. 
