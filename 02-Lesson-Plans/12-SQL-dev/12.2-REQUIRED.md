...
* Open `03-SQL-Constraints/Solved/db/schema.sql` in your IDE and explain the following: 
  ...
  * 🔑 In our `books` table, we want our `price` field to link to our `prices` table. We use the `FOREIGN KEY` restraint to set the `books` price field to link to another table. 
