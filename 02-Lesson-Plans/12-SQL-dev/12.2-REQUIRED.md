...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We will need to define relationships between our tables so that we can use the `JOIN` clause to combine employee data with other tables, like their departments.
...
### 8. Instructor Review: SQL Constraints  (10 min) 
...
* Open `03-SQL-Constraints/Solved/db/schema.sql` in your IDE and explain the following: 
  ...
  * 🔑 We use the `PRIMARY KEY` constraint to set up the unique identifier for the table. It is often the `id` field of our table. It is important that the field that is designated as the `PRIMARY KEY` has unique data for each row. We use `AUTO-INCREMENT` to generate a new, and unique id for each row automatically. 
  ...
  * 🔑 We use the `REFERENCE` key to define the table and `PRIMARY KEY` that we want our book's `price` table to link to. We always link a `FOREIGN KEY` to a `PRIMARY KEY` in another table. 
...
* Navigate to `05-Prepared-Statements/Solved` in your command line and run `npm install` and `node server.js` to demonstrate the following:
  ...
  * 🔑 The prepared statement using a `?` to stand in for the id's value. 

    ```sql
    DELETE FROM books WHERE id = ?
    ```

  * 🔑 For our `DELETE` to work, we then provide the value we want inserted in place of the `?` in our `query()` function. This value is often a variable such as `deletedRow`.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We used prepared statements in Lesson 4 when building our CRUD routes. 
