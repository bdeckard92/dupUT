...
* Navigate to `03-SQL-Constraints/Solved/` in the command line and demonstrate the following:
  ...
  * 🔑 We add a `NOT NULL` constraint to require each row of our table to have a value for a column. In the `books` table, both the `id` and `book_name` must have a value. 

  * 🔑 We use the `Primary Key` constraint, noted by `PRI`, to identify which column will be used to uniquely identify each row.

  * 🔑 We use the `Foreign Key` constraint, noted by `MUL`, to identify a column from a table that we will use to link two tables together. 

  * 🔑 A foreign key always references the primary key of another table. We use the `REFERENCE` clause to show that relationship.     
...
* Open `03-SQL-Constraints/Solved/db/schema.sql` in your IDE and explain the following: 
  ...
  * 🔑 We use the `REFERENCE` key to define the table and `PRIMARY KEY` that we want our book's `price` table to link to and define the relationship between tables. We always link a `FOREIGN KEY` to a `PRIMARY KEY` in another table. 
