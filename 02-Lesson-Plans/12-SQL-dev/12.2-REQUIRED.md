...
* Navigate to `05-Prepared-Statements/Solved` in your command line and run `npm install` and `node server.js` to demonstrate the following:
...
* Open `05-Prepared-Statements/Solved/server.js` in your IDE and explain the following: 
  ...
  * 🔑 To execute the `DELETE`, the prepared statement must be supplied a value. The `deletedRow` variable has a value of `2`. To execute our query, we add the variable name as the second argument for our query function, so that `id = 2`. 
