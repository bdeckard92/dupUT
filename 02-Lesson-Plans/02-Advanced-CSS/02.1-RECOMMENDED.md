...
- Ask the class the following question (☝️) and call on students for the answer (🙋):
    ...
    * 🙋 Make it unique. Add custom styling to make it unique.

- Acknowledge to students that not everybody is a designer. So it's okay to draw inspiration from existing resources.

- Navigate to the following pages and introduce students to publicly available resources that can help them with their design. 
...
- The following questions more closely represent the kinds of questions students might get in an interview and were drawn from this week's Career Connection. Ask students the question (☝️) and call on students for answers (🙋):

    * ☝ Where can styles be defined?

    * 🙋 Styles can be defined in mainly in three different places:
    
        * Inline—used to style only a small piece of code: `<div style="background-color: #cc0000"> This is a section </div>`

        * Embedded—style sheets are put inside the `<head>` element in between the `<style>` tags. For example, `<style> div{ background-color: #cc0000 } </style>`

        * External—style rules are defined in a separate file and linked from the HTML file using `<link rel="stylesheet" type="text/css" href="./assets/css/app.css" />`
