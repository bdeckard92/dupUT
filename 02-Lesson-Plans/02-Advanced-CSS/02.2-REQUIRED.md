...
* Also provide students with the [mockup.png](../../01-Class-Content/02-Advanced-CSS/01-Activities/3-Stu_CSS-Grid/Images/100-mockup.png) image.
...
* Open `03-CSS-Grid/Solved/assets/css/style.css` in your IDE and explain the following:
  ...
  * 🔑 Now that we have our basic layout, let's target elements and make them occupy the spaces specified in the instructions. Starting with grid-item 10, lets make it two rows spanning three columns using `grid-column` and `grid-row` rules.
...
* Open `04-Pseudo-Classes/Solved/assets/css/style.css` in your IDE and explain the following:
  ...
  * 🔑 We use `:hover` to specify how the styles will change when the mouse is hovering over the item, in our case the list item.
...
* Open `05-CSS-Variables/Solved/assets/css/style.css` in your IDE and explain the following:
  ...
  * 🔑 We select the `:root` element. Then we take a color and assign it to a variable using the syntax `--varName: value;`. For example, we take the #F9EDED and place it into a variable named `--light`. 
