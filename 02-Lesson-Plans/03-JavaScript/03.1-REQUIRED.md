...
* Direct students to the activity instructions found in `01-Conditional-Statements/README.md`, which are also shown here:.
	...
	* It is done when the message "False ❌ False ❌" is logged when both `expression1` and `expression2` are false. 
	...
	If you have completed this activity and want to further your knowledge, work through the following challenge with your partner:
...
* Open `01_Conditional-Statements/Solved/script.js` in your IDE and explain the following:
  ...
  * We can evaluate whether both expressions are `true` using the logical `&&` operator.
...
* Open `02-Iteration/Solved/index.html` in your IDE and explain the following:
  ...
  * We then use the `length` property to find the total number of elements in the `students` array. This `length` property will come in handy when we set the condition for the loop.

    ```js
    students.length
    ```

  * 🔑 We write the `for` loop. We set the counter to `0` and dictate that the loop will run till the counter equals the length of the array.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Use student answers to transition to the next section.
