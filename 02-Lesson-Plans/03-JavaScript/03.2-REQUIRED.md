...
* Open `03-Functions/Solved/index.html` in your IDE and demonstrate the following:
  ...
  * 🔑 One way we can define a function is to use a **function declaration**. A function declaration starts with the keyword `function` preceding a name and parentheses `()`.

    ```js
    function isEqual(x, y)
    ```

  * 🔑 The block of code we want to execute goes in curly brackets `{}`. After the code we want to execute, we use `return`. The return statement stops the execution of the function.
  ...
  * 🔑 Functions can also take parameters! Parameters allow us to pass values into a function by giving the values a name. The values we want to use in the code block are given the names `x` and `y`.
...
* Open `03-Functions/Solved/script.js` in your IDE and explain the following:
  ...
  * We use an `if` statement to evaluate whether the values are strictly equal. If so, the message `"They are equal in type and value"` will log.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to the lessons, read the [MDN Web Docs on functions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Functions), and stay for Office Hours to ask for help.
...
* Open `04-Scope/Solved/index.html` in your IDE and demonstrate the following:
  ...
  * 🔑 Declaring a variable inside a function changes the scope of the variable. The variable can only be accessed inside that function. This is called **local scope**.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to the lessons, read the [MDN Web Docs on scope](https://developer.mozilla.org/en-US/docs/Glossary/Scope), and stay for Office Hours to ask for help.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to the lessons, read the [MDN Web Docs on this](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/this), and stay for Office Hours to ask for help.
