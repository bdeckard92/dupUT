...
* Open the lesson's [slide deck](https://docs.google.com/presentation/d/1mgyA1oflkdedI0NfTkh-fHyhjNnKsWl-jMRiaQmARU0/edit?usp=sharing) and walk through the following slides on foundation setting: 
    ...
    * This week, the students will learn about Progressive Web Applications (PWA). Ask the class how many of them would like to go work for companies that develop applications that have a global audience. You'll probably get a decent amount of students that respond yes.

    * Ask the class the following questions (☝️) and call on students for the answers (🙋):

        * ☝️ So if we're developing applications for a global audience, what are some things we need to think about?

        * 🙋 Not everybody has access to the same level of internet. For example, in some developing countries, speeds might be the equivalent to 3G or slower. Additionally, access via a desktop or laptop computer might be limited, with consumers of your application using only mobile devices like phones and tablets to access the Internet.

        * ☝️ And what does that mean for us? Why do we need to think about that?

        * 🙋 We need to develop applications that are accessible to a wide range of audiences, for those on different devices as well as those who have different connectivity levels. Progressive Web Applications, or PWAs, are just one way to reach an audience with lower internet speeds than those we might be accustomed to.

    * Let students know that PWAs play an important role in developing applications for a global audience. Even in the countries with faster internet speeds, there are times when connectivity is low, i.e. if you are in rural areas or on an underground train, so PWAs have huge benefits and it's something that can bring a lot of value to a future employer.
...
* Navigate to [https://secure-bayou-27137.herokuapp.com/](https://secure-bayou-27137.herokuapp.com/) in your Chrome browser and demonstrate the following:
    ...
    * 🔑 This looks like a regular web application, but if we go to Settings and click "Install Images App", we are able to install it as a desktop app.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to the lesson plan, read the [MDN web docs on PWAs](https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps), and stay for Office Hours to ask for help.
