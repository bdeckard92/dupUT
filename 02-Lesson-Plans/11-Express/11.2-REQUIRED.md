...
* In the async material, jQuery is introduced in Lesson 3 where students are tasked with adding drag and drop functionality to the Taskmaster Pro webpage.
...
* Open the Insomnia application and demonstrate the following:
    ...
    * 🔑 Enter a new character's details (feel free to change):
...
* Use the prompts and talking points below to review the following key (🔑) points:
    ...
    * ✔️ POST route in `server.js`

    * ✔️ Using Insomnia to send a POST request

* Open `/03-Stu_POST_Server/Solved/server.js` in your IDE and explain the following:
    ...
    * After logging the new character in the console, we push it to the characters array. Finally we use `res.json` to send a response to the client.
    ...
    * 🔑 To test this route, we will POST to `localhost:3000/api/characters`, using the following data in Insomnia:
...
* Navigate to `04-Stu_Static/Solved/` in your command line and run `node server.js`.
...
* Direct students to the activity instructions found in `/04-Stu_Static/README.md`.
...
* Navigate to `04-Stu_Static/Solved` from the command line and run `node server.js`.
...
* Navigate to `05-Stu_POST-Client/Solved` in your command line and run `node server.js`
...
* Direct students to the activity instructions found in `05-Stu_POST-Client/README.md`.
...
* Open `05-Stu_POST-Client/Solved/server.js` in your IDE and explain the following:
    ...
    * 🔑 In our POST route, `req.body` is equal to the JSON post sent from the user
    ...
    * We use the regular expression provided in the instructions to remove all spaces from the route name. 
