...
* Each activity will require spinning up a new Express.js server. Students might encounter a common error with the code `EADDRINUSE`, due to the port being used. They can easily fix this error by running one of the following commands depending on their operating system:
...
* Open `02-Challenge/Main/index.html` in your browser and explain the following:
  ...
  * This week's Challenge is a "Note Taker" app that allows us to add notes and save them to a JSON file. It also makes the data persist, which means that anytime we want to view our notes, they'll be there, even if we had to restart the server.
...
* Open Insomnia, create a new POST request to `http://localhost:3001/api/reviews`, and demonstrate the following:
  ...
  * 🔑 In the terminal, a few console logs will also provide us with additional information about the client.
  ...
  * 🔑 Remember that when creating POST routes, requests will also need to be concluded to prevent the client requests from timing out and letting them know that the server is done processing their request.
