...
* In Lesson 1, students are introduced to Express, GET routes, and `req.params` to serve up animal data from a JSON file. 
...
* Navigate to `01-Routes/Solved/` in the command line and run `node server.js` to demonstrate the following: 
...
* Navigate to `02-Parameters/Solved/` in your command line and run `node server.js` to demonstrate the following: 

    * 🔑 In our browser, when we visit `http://localhost:3000/api/characters`
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We used this in the first lesson of the module project when we created routes to serve all animal data or serve just one animal's data by its `id` property.
