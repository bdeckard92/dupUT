...
* Open your IDE and review the solution to the CodeSignal algorithm:
    ...
    * In this algorith challenge, you are given an array of `integers` representing the digits of a positive integer. For example, `digits = [1, 2, 3]` represents the integer 123. It is guaranteed that digits does not have any leading zeros. Assuming that digits represents the integer `n`, your task is to return an array that represents the integer `n + 1`.
...
* Ask the class the following questions (☝) and call on students for answers (🙋):
    ...
    * 🙋 Same format as a regular for loop, but starting at the end, and then decrementing `i`.
    ...
    * ☝ Exactly! Then what we'll want to do in this for loop is increment the element, i.e. if the last element is `3`, we'll increment it to `4` and so on. But we need some additional checks! We want to check it the element is less than 10 and not the left most element. How can we do that?

    * 🙋 Start off by just incrementing the element with `digits[i]++` and then implementing the following checks in code"
