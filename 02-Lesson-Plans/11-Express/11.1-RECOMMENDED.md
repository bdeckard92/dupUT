...
* Navigate to `02-Parameters/Solved/` in your command line and run `node server.js` to demonstrate the following: 
    ...
    * 🔑 In our browser, when we visit `http://localhost:3000/api/characters`, we see all of our character data as JSON.

    * 🔑 When we visit `http://localhost:3000/api/characters/yoda`, `/api/characters/darthmaul`, or `/api/characters/obiwankenobi`, we only see one character's data as JSON.
