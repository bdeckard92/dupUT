...
* Open Insomnia Core and demonstrate the following:
  ...
  * 🔑 When we make a POST request to `/submit` we format the data for a new user in JSON, i.e.
...
* Open Insomnia Core and demonstrate the following:
  ...
  * In the response we get back, we can see the notebook with a note as a subdocument.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We used subdocuments to add replies to the comments in Lesson 3. 
...
* Open `02-Subdocuments/Solved/models/Notebook.js` in your IDE and explain the following:
  ...
  * We are requiring the `noteSchema` and notice how we have included it in the `notebookSchema`.
...
* Open `02-Subdocuments/Solved/server.js` in your IDE and explain the following:
  ...
  * We query the `Notebook` collection to find the specific `notebook` that the note is being created for.
  ...
  * We query the `Notebook` collection to find the specific `notebook` that has the `note` we are trying to delete.
