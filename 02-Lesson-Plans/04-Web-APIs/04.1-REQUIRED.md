...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We reviewed how to create HTML using DOM methods and listen for clicks on elements using `addEventListener()`.
  ...
  * 🙋 We covered basic DOM manipulation and `addEventListener()` in Lesson 1.
...
* Expect that students may ask for help with the following:
