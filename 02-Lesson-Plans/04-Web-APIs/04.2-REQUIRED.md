...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We would first access the element using `document.querySelector()`, and assign it to a variable. Once we have that variable, we can access the data-attribute on the `dataset` property using dot notation.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Values stored in `localStorage` are stored locally, in the users browser.
