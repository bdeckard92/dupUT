...
* In the module project, students will work with `localStorage` in the final lesson when they add persistence to their to-do list. They also worked with it lightly in Module 03, so they should have a general idea of how to use it.
...
| 8:15PM | 9   | Instructor Demo: Question Game  | 0:05     |
| 8:20PM | 10  | Student Do: Question Game       | 0:20     |
| 8:40PM | 11  | Instructor Review: Question Game| 0:10     |
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to the lesson plan, read the [`setInterval()` docs on MDN](https://developer.mozilla.org/en-US/docs/Web/API/WindowOrWorkerGlobalScope/setInterval), and stick around for Office Hours to ask for help.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to the lesson plan, read the [`localStorage` documentation on MDN](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage), and stick around for Office Hours to ask for help.
...
* Open `05-Stu_Question-Game/index.html` in your IDE and explain the following: 
    ...
    * 🔑 We first declare an array of objects containing question / answer pairs and initialize a global `score` variable.
    ...
    * 🔑 We then iterate over every object in the `questions` array and create a `confirm()` querying the user for their answer to each question. We then check if their answer is true or false and adjust the score accordingly. 
    ...
    * At the end of the game, the player receives and alert with their final score. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to the lesson plan, read the [`Object` documentation on MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object), and stick around for Office Hours to ask for help.
