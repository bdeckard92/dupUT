...
* Open `03-Timers-Intervals-Intervals/Solved/index.html` in your browser and demonstrate the following:
...
* Direct students to the activity instructions found in `03-Timers-Intervals-Intervals/README.md`.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Data attributes are a way to store additional information associated with vanilla HTML.

  * ☝️ How would we access the data-attribute in JavaScript??

  * 🙋 We would first access the element using `document.querySelector()`, assign it to a variable. Once we have that variable, we can access the data-attribute on the `dataset` property using dot notation.
...
### 9. Instructor Demo: Local Storage  (5 min)
...
* Open `05-Local-Storage/index.html` in your browser and demonstrate the following:
  ...
  * Once we enter in our name, email and choose a password we can click "Submit".

  * After filling out the form, open the developer console in the browser.
  
  * 🔑 Click on the "application" tab and then the "storage" tab and look for "local storage". You will notice a new object inside our local storage contain the information we entered in the form.
...
### 10. Student Do: Local Storage  (20 min)
...
### 11. Instructor Review: Local Storage  (10 min)
