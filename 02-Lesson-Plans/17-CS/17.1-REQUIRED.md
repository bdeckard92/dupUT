...
* Navigate to `01-Console/index.html` in the browser and open Chrome DevTool's console:
  ...
  * 🔑 There is also an uncaught reference error in the console noting the counter module is undefined. This error happens because we are running our code in the browser and modules only work in node.js. Since we need to run our tests in a Node environment, we can ignore this error for now.
...
* Open `14-Stu_Closures/Solved/index.js` in your IDE and explain the following:
  ...
  * If we type `npm install` and `npm test` in the command line, we should see the following:
