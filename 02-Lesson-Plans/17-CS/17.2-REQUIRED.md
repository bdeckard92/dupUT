...
* Benchmarking is covered in Lesson 3 and again in Lesson 5. It is the bonus activity for the `04-Fibonacci` activity, but it might take a lot of time to explain in class. So let the students who attempted the bonus know that you will go over the solution during Office Hours. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We learned about the computer science behind JavaScript, including performance measurement, data structures, and algorithms. We also practiced describing our problem-solving process using pseudocoding. We went over closures, factory functions, and event delegation. We are moving fast compared to traditional learners. It’s okay to feel overwhelmed as long as you don’t give up.
...
* Open the [README.md](../../01-Class-Content/17-CS/02-Challenge/README.md) for this week's challenge and go over the following:
    ...
    * This challenge is to create a gist on GitHub that will be a tutorial on regex. You will need to familiarize yourself with regex, so later in the class, we will do an activity on regex to help you understand and use regex.

    * You will need to clone the starter code and edit the contents to create your tutorial on regex. The starter code is a template with a title, introductory paragraph, summary, and table of contents. The table of contents should link to sections of the tutorial that describe the functionality of each component in the regex. Be sure to rename the template to a unique name that describes your tutorial.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to Lesson 2, read the @TODO [{ DOCS }]({ URL }), and stick around for office hours to ask for help.
...
* In Lesson 5, students were introduced to the Fibonacci sequence, another common algorithm question in the interview process. They learned how to pseudocode the steps they would take to code this. In this activity, they will actually implement it.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to Lesson 5, read about some [Common JavaScript Algorithm Interview Questions, Explained](https://medium.com/better-programming/common-javascript-algorithm-interview-questions-explained-68d8c4186270), and stick around for office hours to ask for help.
...
* Navigate to [RegExr](https://regexr.com/) in your browser and demonstrate the following:
    ...
    * 🔑 Here we see a paragraph of text, and on top of the screen there is an Expression. This expression is looking for all words that start with a capital letter. 

    * 🔑 On the left menu, there is a RegEx Reference that you can use to build your own regex. 
...
* Open `05-Regex/Solved/regex.md` in your IDE and explain the following: 
    ...
    * 🔑 Then we use the `[]` to enclose the types of characters we are matching. In our case, that's any lowercase letter `a-z`, any number between `0-9`, underscore `_`, and a hyphen `-`.
...
### 12. GitHub gist (5 min)

* For this challenge, instead of creating a repository on GitHub, students will create a gist. Although it is new to the students, reassure them that it is easy and not so different from creating README markdown files that they are used to.

* Navigate to the [gist home page](https://gist.github.com/) and demonstrate the following:

    * After you have cloned the starter code and finished writing the tutorial, sign in to GitHub and navigate to the [gist home page](https://gist.github.com/).

    * Type in the name and description of your tutorial.

    * Insert the text of your tutorial into the gist text box.

    * Finally, click the `Create public gist` button to create your gist. It's that simple!
...
* Open `04-Fibonacci/Solved/fibonacci.js` in your IDE and explain the following: 
    ...
    * If you attempted to benchmark your solution, here is how we can go about doing that.
...
* Open `04-Fibonacci/Solved/index.js` in your IDE and explain the following: 
    ...
    * 🔑 Finally we set up our benchmark
