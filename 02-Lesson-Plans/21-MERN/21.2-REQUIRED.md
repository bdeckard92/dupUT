# Module 21 Virtual Class (Required): MERN

## Overview 

In today's required virtual class, students will create GraphQL mutations, make query requests from the client side, and use the `useEffect` Hook to make HTTP calls using `fetch`.
...
* Make query requests from the client side using Apollo Boost
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 Since GraphQL is a fairly new concept in the web development community, it will show employers how updated your skill set is.
...
* Navigate to `03-GraphQL-Mutations/Solved` in your command line and run `npm install` and `npm run watch` to demonstrate the following:
    ...
    * 🔑 When we look at the GraphQL Playground running on <http://localhost:3001/graphql> in the browser, we can test the `addBook()` mutation by entering this in the left pane:
    ...
    mutation addBook($title: String!, $author: String!, $pages: Int!) {
      addUser(title: $title, author: $author, pages: $pages) {
        _id
        title
        author
        pages
    ...
    * 🔑 Just to double check, we can run a query on all books to check if the new book is included:
...
* Navigate to `05-useEffect-fetch/Solved` in your command line and run `npm install` and `npm start` to demonstrate the following:
    ...
    * 🔑 Before we search for anything, we see a GIF of a dog when the page loads.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We didn't use `useEffect` in this module, but if you remember, we learned it in Module 20 briefly and we will use it again in the next module. Also, this Hook is used in the Challenge, so use this activity as an opportunity to practice and familiarize yourself with it so that you can succeed in the challenge.
    ...
    * 🙋 You will use `useEffect` to run the `handleSearchGiphy()` function so that it will load GIF on to the page when it loads.
