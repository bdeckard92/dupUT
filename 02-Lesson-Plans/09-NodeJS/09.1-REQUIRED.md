...
In today's virtual class, students will be introduced to ES6 syntax and review the `fs` and `inquirer` packages for Node.js.
...
* Navigate to `01-Arrow-Functions/Solved/` in your command line and run `node index.js Hello` to demonstrate the following:
