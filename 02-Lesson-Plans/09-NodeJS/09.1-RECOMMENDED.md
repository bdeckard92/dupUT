...
* Run `01-File-System/Solved/index.js` with the word "Hello" as an argument from the command line and demonstrate the following: 

    * 🔑 When we run `index.js` with the word "Hello", we see a confirmation message "Done!" along with the word we provided, "Hello".
...
* Use the prompts and talking points below to review the following key (🔑) points:
    ...
    * ✔️ Writing files using the `writeFile` method

    * ✔️ Reading files using the `readFile` method

* Open `01-File-System/Solved/index.js` in your IDE and explain the following: 

    * 🔑 In order to use modules, we need to import it into our application using `require`:

    ```
    const fs = require("fs");
    ```

    * So here we import the `fs`, or file system module, using `require` and store it in a variable named `fs`.
...
* Run `02-Inquirer0Users/Solved/index.js` from the command line and demonstrate the following: 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We used this to formulate profile questions in Lesson 3 on `npm`. 
...
* Direct students to the activity instructions found in `02-Inquirer-Users`.
...
* Use the prompts and talking points below to review the following key (🔑) points:
    ...
    * ✔️ Our application requires two dependencies, `fs` (which was given) and `inquirer`.

    * ✔️ We use three `type` properties to prompt our user for an `"input"`, `"checkbox"` and a `"list"`. The `"checkbox"` and `"list"` require a `choices` property and a corresponding array of options.

    * ✔️ We chain the given callback method to the prompt by using `.then`

* Open `02-Inquirer-Users/Solved/index.js` in your IDE and explain the following: 

    * 🔑 Our application requires two dependencies, one third-party (`inquirer`) and the other standard library (`fs`).
    ...
    * In this way we pass the output of the prompt method to our Promise callback as data.
