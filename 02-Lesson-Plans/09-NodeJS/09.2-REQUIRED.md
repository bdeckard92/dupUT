...
In today's required virtual class, students will review ES6 features, such as arrow functions, functional loops, and object destructuring. 
...
* Implement how and when to use functional loops, such as `.map` and `.filter`
...
* Run `03-Arrow-Functions/Solved/index.js` from the command line and demonstrate the following: 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We need to keep in mind the instances when you should and should not be using arrow functions. Also remember the proper syntax for arrow functions. 
...
* Direct students to the activity instructions found in `03-Arrow-Functions`.
...
* Use the prompts and talking points below to review the following key (🔑) points:
    ...
    * ✔️ Omit the parentheses if there is only one parameter

    * ✔️ Omit an arrow function's curly braces if the arrow function body only contains one expression. 

    * ✔️ Don't use arrow functions when working with objects
...
* Open `03-Arrow-Functions/Solved/index.js` in your IDE and explain the following: 
    ...
    * Since `this` inside of the arrow function will refer to whatever object the one we're defining is inside of, it is important to remember that we should only use arrow functions when suitable. 

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

    * ☝️ How can we remember whether we can use arrow functions or not?

    * 🙋 In most cases, we can replace regular functions with arrow functions. But arrow functions shouldn’t be object methods.

    * ☝️ What can you do if you don't completely understand this?

    * 🙋 We can refer to Lessons 1-3, read the [MDN docs on arrow functions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions), and stick around for office hours to ask for help.
...
* Run `04-Functional-Loops/Unsolved/map.js` from the command line and demonstrate the following: 
...
* Run `04-Functional-Loops/Unsolved/filter.js` from the command line and demonstrate the following: 
...
* Open `04-Functional-Loops/Unsolved/filter.js` in your IDE and explain the code:
    ...
    * 🔑 We can see that the `originalArray` used a `.filter` method to iterate through it and return all of the even numbers. 
...
* Direct students to the activity instructions found in `04-Functional-Loops`.
...
* Use the prompts and talking points below to review the following key (🔑) points:
    ...
    * ✔️ `.map` method will iterate through the array and execute a function for each element.

    * ✔️ `.filter` method will iterate through the array and only return the elements that are truthy.
...
* Open `04-Functional-Loops/Solved/map.js` in your IDE and explain the following: 
    ...
    * 🔑 The callback function is run once for each element in the array. Whatever it returns is what goes into the corresponding index of the new array.
...
* Open `04-Functional-Loops/Solved/filter.js` in your IDE and explain the following:
    ...
    * 🔑 If the function returns something truthy, that array element is copied to the new array; otherwise it is skipped.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 When looking for a word in the dictionary, you're filtering out all the words that don't contain the same letters as the one you're looking for.

    * 🙋 When shopping for pants, you're filtering out all the ones on the rack that aren't your size.

    * 🙋 When you're looking for your friend in a crowd, you're filtering out everyone that doesn't look like them.

    * ☝️ What is a real life example of when you need to map over something?

    * 🙋 When you're mapping through your To-Do List and marking off all the things you've completed for the day.
