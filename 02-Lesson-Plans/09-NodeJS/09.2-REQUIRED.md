...
* Open `03-Arrow-Functions/Solved/index.js` in your IDE and explain the following: 
    ...
    * Arrow functions have a different understanding of what the `this` keyword is referring to at times, so in this case, we wouldn't want too use arrow functions.
...
* Run `05-Obj-Destructuring/Solved/index.js` from the command line and demonstrate the following: 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 @TODO We used this to destructure `projects` and `about` data from `templateData` based on their property key names in Lesson 4.
...
* Direct students to the activity instructions found in `05-Obj-Destructuring`.
...
* Use the prompts and talking points below to review the following key (🔑) points:
    ...
    * ✔️ Destructure data from objects based on their property key names

    * ✔️ Destructure data from arrays based on their index

    * ✔️ Destructure data from strings using `.split`
