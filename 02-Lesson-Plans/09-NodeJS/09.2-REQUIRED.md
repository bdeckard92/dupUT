...
* Students learned about the `fs` package in Lesson 2. The functional loops, `.map()` and `.filter()`, and object destructuring are taught in Lesson 4
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We need to use the `appendFile()` method to capture the user's input and write it to file.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How comfortable do you feel `fs.appendFile()`? (Poll via Fist to Five, Slack, or Zoom)
...
* Open `03-FS-Module/Solved/index.js` in your IDE and explain the following:
  ...
  * 🔑 The first argument is the path; the filename of where to append the text. If the file doesn't exist yet, it will create it. The second argument is the data which is the string of text to append to the file. Here we used `${process.argv[2]}`. And finally we have the callback function.
  ...
  * 🔑 Notice in the callback function, we used a ternary operator instead of an `if` statement. A ternary operator takes in a condition followed by a question mark (?), then an expression to execute if the condition is truthy followed by a colon (:), and finally the expression to execute if the condition is falsy.
...
* Open `04-Functional-Loops/Solved` in your IDE and explain the following:
  ...
  * 🔑 In the following example, the first array should contain only even numbers, the second array should contain only prime numbers, and the third should contain only numbers larger than `5`:
...
* Navigate to `05-Destructuring-Assignment/Solved/` in your command line and run `node index.js` to demonstrate the following:
...
* Direct students to the activity instructions found in `05-Destructuring-Assignment/README.md`.
