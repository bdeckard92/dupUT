...
* Open `02-Parse-JSON/Solved/index.html` in your browser and demonstrate the following:
  ...
  * 🔑 When the page loads, we see some instructions that us to make a request to get 5 GitHub issues associated with Twitter's Chill repo.
  ...
  * 🔑 We can see the results by opening the develop console in the browser.
...
* Direct students to the activity instructions found in `02-Parse-JSON/README.md`, which is also shown here:
...
* Divide students into breakout rooms. Because the first part of this activity is a group-based activity, **only one person** in the group needs to make the repository for this activity. That person will invite the other developers to join the repo as a collaborator once they have everything set up. Make sure that person is sharing their screen so everyone in the group can see the process of creating and setting up the repo.
...
* If time allows, set up this demonstration yourself, with other instructional staff or students as collaborators, and walk through the guide to demonstrate the following:
  ...
  * When we want to start work on a new feature, we create a new branch, as follows:
    ...
    # or like this...
    git branch super_feature
    git checkout super_feature
    ```
