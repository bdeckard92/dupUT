...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 In Lesson 3, students made dynamic calls to the GitHub API using user input.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 In the async material, students used query parameters in their API requests to sort the list of issues from GitHub in ascending order. Students also parsed the query URL to retrieve the query parameter to make dynamic APIs requests in Lesson 4.
