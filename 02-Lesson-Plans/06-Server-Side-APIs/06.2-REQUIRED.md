...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We learned how to make requests to a public server-side APIs using the `fetch()` function, display the corresponding response using JavaScript, and use query parameters to transfer information between html pages. We are moving fast compared to traditional learners. It’s okay to feel overwhelmed as long as you don’t give up.
