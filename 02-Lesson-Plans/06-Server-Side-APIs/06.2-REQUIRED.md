...
* Open `04-Deconstruct_Parameters/Solved/index.html` in your browser and demonstrate the following:
  ...
  * 🔑 For example, we can use the `per_page` parameterlimit the number of results returned by . Another example is the `since` parameter, which allows us to return results starting from a specific date.
