...
* Open the [script.js](Unsolved/assets/js/script.js) file and do the following:
  ...
  * Make a `fetch` request using the user's input

  * In the callback function, create a variable that selects the `<div>` where the GIF will be displayed
...
* Open `03-Gif-Search/Solved/assets/js/script.js` in your IDE and explain the following: 
    ...
    * We create a function called `myFunction()`. We do this so that this function can be executed when the search button is clicked. 

    ```js
    function myFunction() {}
    ```

    * 🔑 Next, we capture our user's input. We do this by using `document.querySelector()` to target the ID of the input, and use `.value` to capture the value of the input, storing it in a variable. 

    ```js
    var searchTerm = document.querySelector("#searchTerm").value;
    ```

    * 🔑 Now we setup our `fetch` request, concatenating our `searchTerm` into the query url. Remember to add your API key at the end.

    ```js
    fetch('https://api.giphy.com/v1/gifs/search?q=' + searchTerm + '&api_key=HvaacROi9w5oQCDYHSIk42eiDSIXH3FN&limit=1')
    ```

    * Next, we select the `<div>` where we will display the GIF. We empty the div on each search so we do not just append a GIF every time.
    ...
    * Then we create a `<img>` element. We need to specify the index of the `data` response that we want.
    ...
    * 🔑 Finally we append the GIF to the cleared div.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to Lesson 3, read the [MDN Docs on Object.value](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_objects/Object/values), and stick around for office hours to ask for help.
...
* Open the [script.js](Unsolved/assets/js/script.js) file and do the following:
  ...
  * Modify the `fetch` request with a `rating` parameter.
...
* Open `04-Query-Params/Solved/index.html` in your IDE and explain the following:

    * 🔑 First we need to create a `<select>` menu with an id of `rating`. Then we give it 3 options of G, PG, and PG-13:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to Lesson 4, refer to the Giphy search [API endpoint docs](https://developers.giphy.com/docs/api/endpoint#search), and stick around for office hours to ask for help.
...
* Open the [script.js](Unsolved/assets/js/script.js) file and do the following:
  ...
  * Make a fetch request to the Giphy search API with the article title and rating parameters. Remember to add your API key at the end.
...
* Open `05-Nested-API-Calls/Solved/assets/js/script.js` in your IDE and explain the following: 
    ...
    * We target the HTML element with the ID of `#response-header`, which we will then change the innerHTML to show the article title.
    ...
    * 🔑 Finally, we `return` a `fetch` request to the Giphy API, including the `searchTerm` and `rating` parameters we stored earlier.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to Lesson 2 and stick around for office hours to ask for help.
