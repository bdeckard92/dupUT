...
* In the async material, students are introduced to the `mysql2` npm package in Lesson 2 when students create API routes for their candidate data. In Lesson 3, they learn about `JOIN` statements to merge candidate data with party data.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We used this to build API routes in Lessons 2.
