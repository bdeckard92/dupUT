...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We learned what foreign keys are, how to use `JOIN` statements, and how to connect to a SQL database from Node.js. We are moving fast compared to traditional learners. It’s okay to feel overwhelmed as long as we don’t give up!
...
* Open `03-SQL-Constraints/Solved/db/schema.sql` in your IDE and explain the following:
  ...
  * 🔑 We use the `PRIMARY KEY` constraint to set up the unique identifier for the table -- often the `id` column of the table. The column that is designated as the `PRIMARY KEY` must have unique data for each row.

  * We use `AUTO-INCREMENT` to automatically generate a new, unique id for each row:
  ...
  * 🔑 We use the `REFERENCE` clause to define the table and the `PRIMARY KEY` that we want the book's `price` table to link to, thus defining the relationship between tables. We always link a `FOREIGN KEY` to a `PRIMARY KEY` in another table:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 The `PRIMARY KEY` constraint provides a unique identifier for each row of data in a table.
...
* Direct students to the activity instructions found in `05-SQL-Prepared-Statements/README.md`.
