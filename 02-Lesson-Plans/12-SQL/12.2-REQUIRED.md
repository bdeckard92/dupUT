...
In today's required session, students will review `JOIN` statements, the `sqlite3` npm package, and `?` prepared statements.
...
* Use the `sqlite3` npm package to create a database connection
...
* We use SQLite instead of MySQL, so the syntax for certain things will be slightly different. For example, SQLite does not support `RIGHT JOIN`.

* The SQLite command line doesn't print data in an easy-to-read format unless you turn on columns and headers with the following commands: `.mode column` and `.headers on`.

* In the async material, the `sqlite3` npm package is first introduced in Lesson 2 when students create API routes for their candidate data. In Lesson 3, they learn about `JOIN` statements to merge candidate data with party data.
