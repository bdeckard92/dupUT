...
In today's required session, students will review `JOIN` statements, the sqlite npm package, and `?` prepared statements.
...
* Use the sqlite npm package to create a database connection
...
* In the async material, the sqlite npm package is first introduced in Lesson 2 when students create API routes for their candidate data. In Lesson 3, they learn about `JOIN` statements to merge candidate data with party data.
