...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Add on a capital letter to the end of the word to indicate a stopping place, and create a holder variable for the current word we're looking at when iterating.
  ...
  * 🙋 Using a `for` loop, iterate through the variable name, and check whether it is a capital letter. If it is, check whether that word exists in the array — if it doesn't return `false`. Else, add it to our `currentWord` placeholder.
