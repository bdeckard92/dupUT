...
In today's recommended virtual class, we will learn about MySQL schemas, creating databases and tables, and populating tables with data, as well as solve a new algorithm in CodeSignal. 
...
* Students should have installed MySQL on their machines along with SQLite. Direct them to the `Up and Running` on Canvas for directions on how to install it if they haven't done so.
...
| 6:55PM | 3  | Instructor Demo: SQL Schema       | 0:05     |
| 7:00PM | 4  | Student Do: SQL Schema            | 0:15     |
| 7:15PM | 5  | Instructor Review: SQL Schema     | 0:10     |
| 7:25PM | 6  | Instructor Demo: SQL Seeds        | 0:05     |
| 7:30PM | 7  | Student Do: SQL Seeds             | 0:15     |
| 7:45PM | 8  | Instructor Review: SQL Seeds      | 0:10     |
| 7:55PM | 9  | Recap                             | 0:05     |
...
* Navigate to `01-SQL-Schema/Solved/` in the command line and initialize the MySQL command line by typing `mysql -u root -p` and your MySQL password. Be sure your MySQL Server is running!
...
* Open `01-SQL-Schema/Solved/schema.sql` in your IDE and explain the following:
    ...
    * 🔑 We create the `books` table with the `CREATE TABLE books` command. Notice we must specify whether it is an `INTEGER` or `VARCHAR`:
    ...
    * 🔑 To initiate the MySQL command line, type: `mysql -u root -p` in your command line and then your MySQL password. We execute the `schema.sql` file in the MySQL command line with the `source` command:
...
* Navigate to `02-SQL-Seeds/Solved/` in the command line and initialize the MySQL command line by typing `mysql -u root -p` and your MySQL password. 

    * 🔑 When we initialize the MySQL command line, `mysql>`, we first set up the database by typing: `source schema.sql`
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * ☝️ Can anyone explain the benefits of seeding a table?
